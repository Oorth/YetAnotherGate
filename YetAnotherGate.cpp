//cl.exe /EHsc .\engine.cpp /link /OUT:engine.exe
/*
...
void* syscall_FindExportAddress(HMODULE, const char*);
...
int GetHookedFunctions()
{
    ...
    if(!sLibs.hKERNELBASE)
    {
        ...
        return 0;
    }//printf("\n[DEBUG] KERNELBASE handle before syscall_FindExportAddress: 0x%p", sLibs.hKERNELBASE);

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    
    fn.MyVirtualProtect = (pVirtualProtect)syscall_FindExportAddress(sLibs.hKERNELBASE, "VirtualProtect");
    if(fn.MyVirtualProtect == nullptr)
    {
        fuk("Failed to get VirtualProtect address"); return 0;
    }norm(GREEN"\t[DONE]");

    fn.MyVirtualAlloc = (pVirtualAlloc)syscall_FindExportAddress(sLibs.hKERNELBASE, "VirtualAlloc");
    if(fn.MyVirtualAlloc == nullptr)
    {
        fuk("Failed to get VirtualAlloc address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyGetLastError = (pGetLastError)syscall_FindExportAddress(sLibs.hKERNELBASE, "GetLastError");
    if(fn.MyGetLastError == nullptr)
    {
        fuk("Failed to get GetLastError address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyGetStdHandle = (pGetStdHandle)syscall_FindExportAddress(sLibs.hKERNELBASE, "GetStdHandle");
    if(fn.MyGetStdHandle == nullptr)
    {
        fuk("Failed to get MyGetStdHandle address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyGetCurrentProcess = (pGetCurrentProcess)syscall_FindExportAddress(sLibs.hKERNELBASE, "GetCurrentProcess");
    if(fn.MyGetCurrentProcess == nullptr)
    {
        fuk("Failed to get MyGetCurrentProcess address"); return 0;        
    }norm(GREEN"\t[DONE]");  
    
    fn.MyNtOpenSection = (pNtOpenSection)syscall_FindExportAddress(sLibs.hHookedNtdll, "NtOpenSection");
    if(fn.MyNtOpenSection == nullptr)
    {
        fuk("Failed to get MyNtOpenSection address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyNtMapViewOfSection = (pNtMapViewOfSection)syscall_FindExportAddress(sLibs.hHookedNtdll, "NtMapViewOfSection");
    if(fn.MyNtMapViewOfSection == nullptr)
    {
        fuk("Failed to get MyNtMapViewOfSection address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyNtClose = (pNtClose)syscall_FindExportAddress(sLibs.hHookedNtdll, "NtClose");
...
{
    norm("\n\n================UpdateFunctions()================");
    fn.MyNtOpenSection = (pNtOpenSection)syscall_FindExportAddress(sLibs.hUnhookedNtdll, "NtOpenSection");
    if(fn.MyNtOpenSection == nullptr)
    {
        fuk("Failed to get MyNtOpenSection address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyNtMapViewOfSection = (pNtMapViewOfSection)syscall_FindExportAddress(sLibs.hUnhookedNtdll, "NtMapViewOfSection");
    if(fn.MyNtMapViewOfSection == nullptr)
    {
        fuk("Failed to get MyNtMapViewOfSection address"); return 0;        
    }norm(GREEN"\t[DONE]");

    fn.MyNtClose = (pNtClose)syscall_FindExportAddress(sLibs.hUnhookedNtdll, "NtClose");
    if(fn.MyNtClose == nullptr)
    {
        fuk("Failed to get MyNtClose address"); return 0;        
    }norm(GREEN"\t[DONE]");
...
void* AddStubToPool(Sys_stb* sEntry, size_t NumberOfElements)
{
    ...
    {
        void* vpfunction = syscall_FindExportAddress(sLibs.hHookedNtdll, sEntry[j].function_name);
        if(!vpfunction)
        {
...
void* InitSyscallGate(Sys_stb* sEntry, size_t NumberOfElements)
{
    ...
    AddStubToPool(sEntry, NumberOfElements);
...
void* syscall_FindExportAddress(HMODULE hModule, const char* funcName)
{
